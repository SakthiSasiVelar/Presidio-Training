...
namespace Shopping_BL_Library
{
    public class CartItemBL : ICartItemService
    {
        ...
        public async Task<int> GetCartItemQuantity(int id)
        {
            try
            {
                CartItem cartItem = await cartItemRepository.GetByKey(id);
        ...
        public async Task<double> GetCartItemPrice(CartItem item)
        {
